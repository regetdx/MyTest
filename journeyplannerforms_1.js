...
document.oncontextmenu=new Function("return false"); // jQuery plugin to prevent double submission of forms 
...
function cancelTicketStart(tdrflag,initFlag,quota,traincancel,softflag)
{
	var flag = false;
	cancelpsgn="NNNNNN";
	var selectCount = 0;
		...
		if (tdrflag) {
			...
			if (reason==0) {
				...
				//	if (flag1) {
						...
						var answer = confirm (cancelTdrMsg["confirm_Cancel_Request"]);	
						if (answer) {
							if(tempformid==0) {
								$("#cancelListForm\\:cancelbuttonid").attr('disabled','disabled');
							}
...
function getPnrEnquiryForCancel(){
	...
	//var t = $("input:radio:checked").parents("tr:first").find("td:eq(1)").text();
		...
		if (!(status=="Booked" || status=="Partial Cancelled")) {
...
function resendBkgSms(){
	...
	if(tr!=null && tr.length==1){
     ...
     if (statusSMS=="Partial Cancelled"   || statusSMS=="Booked" ) {
...
}( jQuery ));// jQuery plugin to prevent double submission of forms
jQuery.fn.preventDoubleSubmission = function() {
  $(this).on('submit',function(e){
    var $form = $(this);
...
$.ui.autocomplete.filter = function (array, term) {
    var max=10;
   var matcher = new RegExp(ui_1_10_4.autocomplete.escapeRegex("- "+term)+"$", "i");
   var matcher1 = new RegExp("^" + ui_1_10_4.autocomplete.escapeRegex(term), "i");
   var matcher2 = new RegExp(ui_1_10_4.autocomplete.escapeRegex(term), "i");
  ...
  if(results.length<max){
     var result1 = $.grep(array, function (value) {
           return (!matcher.test(value.value || value) &&
     ...
     results=$.merge(result, result1);
      ...
      if(results.length<max){ 
         var result2 = $.grep(array, function (value) {
               return (!matcher.test(value.value || value) &&
                       !matcher1.test(value.value || value) &&
                       matcher2.test(value.value || value));
           });
         results=$.merge(results, result2);
      }
  }
...
$(document).ready(function(){
		...
		$(document.getElementById("jpform:fromStation")).autocomplete(autoCompleteOptions);			
		$(document.getElementById("jpform:toStation")).autocomplete(autoCompleteOptions);
		...
		setQbAutoComplete();
		$(document.getElementById("favJourney:fromStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("favJourney:toStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("txnHistory:fromStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("txnHistory:toStation")).autocomplete(autoCompleteOptions);		
		$(document.getElementById("txnCancelHistory:fromStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("txnCancelHistory:toStation")).autocomplete(autoCompleteOptions);		
		$(document.getElementById("cancelListForm:filterfromid")).autocomplete(autoCompleteOptions);
		$(document.getElementById("cancelListForm:filtertoStnid")).autocomplete(autoCompleteOptions);
		$(document.getElementById("failedTransactionHistory:fromStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("failedTransactionHistory:toStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("tdrHistory:fromStation")).autocomplete(autoCompleteOptions);
		$(document.getElementById("tdrHistory:toStation")).autocomplete(autoCompleteOptions);
		$('form').preventDoubleSubmission();		
});

function setJPHindiLabels(){
	$(document.getElementById("jpform:fromStation")).on( "autocompleteselect", 
            function( event, ui ) {
	...
	$(document.getElementById("jpform:toStation")).on( "autocompleteselect", 
	            function( event, ui ) {
	               var newValTxt=ui.item.label.split("-")[0];
	               var hindiJourneyTxt=$("#jpform\\:hindiNameTxt").text();
	               var txtArray=hindiJourneyTxt.split(" - ");
...
function setQbAutoComplete(){
	var autoCompleteOptions={source: stationName, delay:0, autoFocus: true};
	$(document.getElementById("qbform:fromStation")).autocomplete(autoCompleteOptions);
	$(document.getElementById("qbform:toStation")).autocomplete(autoCompleteOptions);
	
	$(document.getElementById("cbform:fromStation")).autocomplete(autoCompleteOptions);
	$(document.getElementById("cbform:toStation")).autocomplete(autoCompleteOptions);
...
$(document).ready(function(){
	...
	handicapPassengers = $("#jpform\\:handicapPassengers").is(":checked");
});
function isJpChanged()
{
	...
	if ( $("#jpform\\:handicapPassengers").is(":checked") != handicapPassengers ) return true;
...
$(document).ready(function(){
	$("#jpform\\:handicapPassengers,#qbform\\:handicapPassengers,#cbform\\:handicapPassengers").change(function(){
		if($(this).is(":checked")){
...
function validatePassengerForm(event,msgs){
	...
	$(".psgn-name").parent("td").parent("tr").each(function(index){
		...
		curtr.find(".psgn-name,.psgn-age,.psgn-gender,.psgn-foodChoice,.psgn-idcardtype,.psgn-idcardnumber,.psgn-disablePsgnType,.psgn-disableCardId,.psgn-disablePsgnDob,.psgn-disableCardExpDt").removeClass("error_field");
		if($.trim(psgnName.val()) != ''){
			...
			if(foodChoiceEnabled){
				...
				if(!(foodChoice.val() == 'V' || foodChoice.val() == 'N')){
			...
			/*
			 * Checks for handicap passengers
			 */
			if(_handicapPassengers){
	...
	if(_handicapPassengers){
		if(hpQuotaBooking){
			...
			if(!handicapPsgnFound){
				msgs.push(validatorMsg["no_HNDCAP_psgn_found"]);
			}
		}
		if(escortPsgnFound && !handicapPsgnFound){
...
function setPassengerInputValidation(){
	...
	$(".psgn-disablePsgnDob").blur(function(){
	    ...
	    if(null !=d && "" != d && isValidDate(d)){
	    	...
	    	if(tr.find(".psgn-disablePsgnType").val() == 1){
	    		var age=getAge(d1[2]+"-"+d1[1]+"-"+d1[0]);
	    		if(validAge(age)){
	    			tr.find(".psgn-age").val(age);
	    		}
	    	}
	    }
	});
	...
	if(seniorCitizenApplicable){
		...
		$(".psgn-concopt:enabled").each(function(){
			...
			if(_handicapPassengers){
			 ...
			 if(!$(this).is(":checked") &&
					 (!_handicapPassengers ||(_handicapPassengers &&  !(dpt == 1 || dpt ==2)))){
...
function selectMasterPassenger(){
	...
	for(i=0,selectpsgn=0;  selectpsgn < selectedPsgns.length && i < maxPassengers; i++){
...
function disableConcessionOpt(){
	if(seniorCitizenApplicable || _handicapPassengers){		
		$(".psgn-name").parent("td").parent("tr").each(function(index){
			var curtr=$(this);
			...
			curtr.find(".psgn-concopt").bind("change",function(){
				disableAutoUpgrade();
				if($(this).is(":checked") && _handicapPassengers ){
				...
				curtr.find(".psgn-disablePsgnType").bind("change",function(){
					...
					if(dpt == 1 || dpt ==2){
						...
						if(dpt == 1){
							...
							curtr.find(".psgn-idcardtype").removeAttr("onfocus").removeClass("readonly");
						}						
					}else{
						curtr.find(".psgn-disableCardId,.psgn-disablePsgnDob,.psgn-disableCardExpDt,.psgn-idcardnumber").val("").attr("readonly","readonly").addClass("readonly");
						curtr.find(".psgn-idcardtype").val("NULL_IDCARD").attr("onfocus","loseFocus(this);").addClass("readonly");
...
function disableAutoUpgrade(){
	if(journeyClass != "1A"){
		if($(".psgn-concopt:enabled:checked").length > 0 || (_handicapPassengers && isHandicapOpted())){
...
function jpBook(thisElem, trnno,fromStnCode,toStnCode,jDate,jClass,jQuota,avlSts,currBkgFlag){
	if(isJpChanged()){
		alert(tbisMsg["changed_jrny_dtls"]);
		return;
	}
...
function validateUserNameLength() { 
         ...
         if(unamecheck.value.length < 6 )  {
            alert("Please enter at least six characters");
            unamecheck.focus();
            return false;
         }
         if(unamecheck.value.length > 10 )  {
            alert("Please enter less than ten characters");
            unamecheck.focus();
            return false;
         }
         ...
         if(upaschecklower.test(unamecheck.value))
         {
      	  ...
      	  alert("UserName must contain at least one small letter");
      	unamecheck.focus();
           return false;
           }
         return true;
...
function fetchStates() {
	var country = document.getElementById("userRegistrationForm:countries").value;
...
function fetchPostOfficeDistrict() {
	var pincode = document.getElementById("userRegistrationForm:postoffice").value;
...
function validate() {
	var unamecheck = $("#masterlist\\:Name"); 
	var uname = $("#userRegistrationForm\\:userName");
    var upas = $("#userRegistrationForm\\:pasword");
    var uconfpas = $("#userRegistrationForm\\:confpasword");
    var illegalChars = /\W/; 
    var illegalCharsPas = /[\W_]/;
    var illegalStreetCheck = /[^A-Za-z0-9\/\.\-\s\,]/;
   ...
   var city = $("#userRegistrationForm\\:cityName");
    ...
    var middleName = $("#userRegistrationForm\\:middlename");
     ...
     var secAnswer = $("#userRegistrationForm\\:secanswer");
     var address = $("#userRegistrationForm\\:address"); 
     var street = $("#userRegistrationForm\\:street");
     var area = $("#userRegistrationForm\\:area");
    var postOffice = $("#userRegistrationForm\\:postofficeName");
     var district = $("#userRegistrationForm\\:district");
     var genderMale = $("#userRegistrationForm\\:gendermsg\\:0:checked");
     var genderFemale = $("#userRegistrationForm\\:gendermsg\\:1:checked");
     var statusMarried = $("#userRegistrationForm\\:msgmarital\\:0:checked");
     var statusUnMarried = $("#userRegistrationForm\\:msgmarital\\:1:checked");
     var dob = $("#userRegistrationForm\\:dobInputDate"); 
     ...
     var dayZeroCheck=Number(d14.val().substr(0,1));
     if(dayZeroCheck==0){
    	var d1 = Number(d14.val().substr(1,2))
     }
     else {
    	 var d1=d14.val();
	     
	     }
     var dayZeroCheck=Number(m14.val().substr(0,1));
     if(dayZeroCheck==0){
    	var m1 = Number(m14.val().substr(1,2))
     }
     else {
    	 var m1 = m14.val();
	     }
    ...
    if(upaschecklower.test(uname.val()))
    {
     ...
     if(uname.val().length > 10 )  {
        alert("Please enter UserName less than ten characters");
        uname.focus();
        return false;
     }
     if ( illegalChars.test(uname.val()))
     {
         alert("Only letters, numbers and underscores are allowed.No space is allowed");
         uname.focus();
         return false;
     } 
    if(upas.val() == '' )  {
  ...
  if(upascheckcase.test(upas.val()))
  {
  }
else{
 ...
 if(upaschecklower.test(upas.val()))
  {
  }
else{
    ...
    if ((upas.val().length < 7 ))
    {
        alert(" Password should be more than seven characters");
        upas.focus();
        return false;
    }
    if ((upas.val().length > 15 ))
    {
        alert(" Password should not be more than fifteen characters");
        upas.focus();
        return false;
    }
    if ( illegalCharsPas.test(upas.val()))
    {
        alert("Only letters, numbers  are allowed for password");
        upas.focus();
        return false;
    }
    ...
    if ( illegalCharsPas.test(uconfpas.val()))
    {
        alert("Only letters, numbers  are allowed for confirm password");
        uconfpas.focus();
        return false;
    }
    if ( upas.val() != uconfpas.val())
    {
    ...
    {
        alert("Please enter at least 3 characters for security answer");
        secAnswer.focus();
        return false;
    }
    if (secAnswer.val().length > 40 )
    {
        alert("Please enter less than 40 characters for security answer");
        secAnswer.focus();
        return false;
    }
    if(firstName.val() == '' )  {
    ...
    {
        alert("Please enter at least 3 characters for first name");
        firstName.focus();
        return false;
    }
    if (firstName.val().length > 20 )
    {
        alert("Please enter less than 20 characters for first name");
        firstName.focus();
        return false;
    }
    ...
    if(middleName.val() != '' )  {
    	if(onlyalpha.test(middleName.val())){ 
	         ...
	         alert("Only letters are allowed in middle name");
	        middleName.focus();
         return false;
       }
      } 
    if(middleName.val() != '' )  {
   	 if (middleName.val().length > 20 )
         {
             alert("Please enter less than 20 characters for middle name");
             middleName.focus();
             return false;
       }
   }     
    ...
    {
        alert("Please enter at least 3 characters for last name");
        lastName.focus();
        return false;
    }
    if (lastName.val().length > 20 )
    {
        alert("Please enter less than 20 characters for last name");
        lastName.focus();
        return false;
    }
    ...
    if($("#userRegistrationForm\\:gendermsg\\:0:checked").length ==0 && $("#userRegistrationForm\\:gendermsg\\:1:checked").length ==0) {
    		    alert("select gender")
    		    return false;		    
    }	
    ...
    if (temail.val().length < 1 )
    {
        alert("Please enter at least 1 characters for email");
        temail.focus();
        return false;
    }
    if (temail.val().length > 50 )
    {
    ...
    {
        alert("Please enter at least 3 characters for Flat No.");
        address.focus();
        return false;
    }
    if (address.val().length > 25 )
    {
        alert("Please enter less than 25 characters for Flat No.");
        address.focus();
        return false;
    }
    if(street.val() != '' )  {
        ...
        {
            alert("Please enter less than 25 characters for street");
            street.focus();
            return false;
        }
 }
    if(area.val() != '' )  {
        ...
        {
            alert("Please enter less than 25 characters for area");
            area.focus();
            return false;
        }
 } 
   ...
   if(country.val() == 1000 && otherCountry.val() != '' )  {     
	   ...
	   alert("Only letters are allowed in other country");
         otherCountry.focus();
         return false;
       }
     }
   if(country.val() == 1000 && otherCountry.val() != '' )  {     
	   if (otherCountry.val().length < 3 )
	     {
	         alert("Please enter at least 3 characters for other country");
	         otherCountry.focus();
	         return false;
	     }
     }
   if(country.val() == 1000 && otherCountry.val() != '' )  {     
	   if (otherCountry.val().length > 25 )
	     {
	         alert("Please enter less than 25 characters for other country");
	         otherCountry.focus();
	         return false;
	     }
     }
   ...
   if(country.val() == 94 )  {     
	         ...
	         {
	         alert("Enter the  Pincode in digits only");
	         pin.focus();
	         return false;
	         }
		     }    
   ...
   if(country.val() == 94 )  {     
  	   if (pin.val().length < 3 )
  	     {
  	         alert("Please enter at least 3 characters for pincode");
  	       pin.focus();
  	         return false;
  	     }
	     }
     if(country.val() == 94 )  {     
  	   if (pin.val().length > 6 )
  	     {
  	         alert("Please enter less than 7 characters for pincode");
  	       pin.focus();
  	         return false;
  	     }
	     }  
     ...
     alert("Only letters and numbers are allowed in pincode");
     pin.focus();
        return false;
     }
		     }    
  ...
  if(country.val() != 94 )  {     
    ...
    if(country.val() != 94 )  {     
 	   if (pin.val().length > 15 )
 	     {
 	         alert("Please enter less than 16 characters for pincode");
 	       pin.focus();
 	         return false;
 	     }
	     }  
  ...
  if(country.val() != 94 && otherState.val() != '' )  {     
      if(onlyalpha.test(otherState.val())){ 
	   ...
	   alert("Only letters are allowed in other state");
	       otherState.focus();
         return false;
      }
     }
  if(country.val() != 94 && otherState.val() != '' )  {     
	   if (otherState.val().length < 3 )
	     {
	         alert("Please enter at least 3 characters for other state");
	      otherState.focus();
	         return false;
	     }
     }
  if(country.val() != 94 && otherState.val() != '' )  {     
	   if (otherState.val().length > 25 )
	     {
	         alert("Please enter less than 25 characters for other state");
	       otherState.focus();
	         return false;
	     }
     }
 ...
 if(country.val() != 94 && otherCity.val() != '' )  {     
     if(onlyalpha.test(otherCity.val())){ 
	   ...
	   alert("Only letters are allowed in other city");
	         otherCity.focus();
         return false;
     }
     }
 if(country.val() != 94 && otherCity.val() != '' )  {     
	   if (otherCity.val().length < 3 )
	     {
	         alert("Please enter at least 3 characters for other city");
	       otherCity.focus();
	         return false;
	     }
     }
 if(country.val() != 94 && otherCity.val() != '' )  {     
	   if (otherCity.val().length > 25 )
	     {
	         alert("Please enter less than 25 characters for other city");
	       otherCity.focus();
	         return false;
	     }
     }
if(country.val() == 94 )  {    
    ...
    {
    alert("Enter the  Mobile Number in digits only");
    mob.focus();
    return false;
    } 
    if (mobZeroCheck == 0 )
 ...
 if(pancardMandateFlg.val() == 'true' && verificationEnabledFlg.val() == 'true' && verificationTypeFlgValue.val()!='NULL_IDCARD' )  {
    	 if(pancardNumber.val() == '' )  {
    		 alert("Please Enter "+verificationTypeFlg.val()+"."); 
    		 pancardNumber.focus();
    		 return false;
    	 }	        
    }    
    if(pancardNumber.val() != '' && verificationTypeFlg.val() == 'PAN Card' && verificationEnabledFlg.val() == 'true' && verificationTypeFlgValue.val()!='NULL_IDCARD')  {
    	if(onlyalphanumeric.test(pancardNumber.val())){ 
        }
       else{
           alert("Only letters and numbers are allowed in PAN Card number");
          pancardNumber.focus();
        return false;
    }
       if (pancardNumber.val().length != 10 )
	      {
   ...
   if(pancardNumber.val().trim() != '' && verificationTypeFlg.val() == 'Aadhaar ID' && verificationEnabledFlg.val() == 'true' && verificationTypeFlgValue.val()!='NULL_IDCARD')  {

  		 if(!mobileExp.test(pancardNumber.val()))
   		 {
    		alert("Only numbers are allowed in Aadhaar Id Number.");
    	    pancardNumber.focus();
     	   return false;
        }
   		 if (pancardNumber.val().length != 12 )
  	      {
  	          alert(" Enter  Valid Aadhaar Id Number.");
  	          pancardNumber.focus();
  	          return false;
  	      }     
   		var pancardNumber =$("#userRegistrationForm\\:pancardno");
	    ...
	    if ( check != 0 ){
	    	 alert("Invalid Aadhaar Id, Please Recheck!");
	    	 pancardNumber.focus();
	    	return false;
	    } 	             
	  }
   ...
   if(pancardMandateFlg.val() == 'true' && verificationEnabledFlg.val() == 'true' && verificationTypeFlgValue.val()=='NULL_IDCARD')  {
   if($("#userRegistrationForm\\:selecIdCard\\:0:checked").length ==0 && $("#userRegistrationForm\\:selecIdCard\\:1:checked").length ==0) {
	    alert("select card")
	    return false;		    
    }	
   }
   if(pancardMandateFlg.val() == 'true' && verificationEnabledFlg.val() == 'true' && verificationTypeFlgValue.val()=='NULL_IDCARD' && selecIdCardpancard.val()=='3')  {
	   if(pancardNumber.val() == '' )  {
   ...
   if(pancardMandateFlg.val() == 'true' && verificationEnabledFlg.val() == 'true' && verificationTypeFlgValue.val()=='NULL_IDCARD' && selecIdCardaadhar.val()=='9')  {
	   if(pancardNumber.val() == '' )  {
    		 alert("Please Enter Aadhar Id Number."); 
    		 pancardNumber.focus();
    		 return false;
    	 }	 
   }
...
if(landLine.val() != ''){
  ...
  {
      alert(" LandLine Number should not be more than 10 digits");
      landLine.focus();
      return false;
  }
 if(!mobileExp.test(landLine.val()))
  {
  alert("Enter the LandLine Number in digits only ");
  landLine.focus();
  return false;
  }
}
...
var r=confirm("Press OK to continue or Cancel for Return");
if (r==true)
  {
	return true;	 
  }
...
function validateForgotPwdDtls() { 
    	var uname = $("#forgot_passwrd\\:userNameForgot");
	...
	var country = document.getElementById("updateUserDtls:countriesProfile").value;
	...
	if(country != 94 || country == 1000){
			...
			document.getElementById("updateUserDtls:postofficeProfile").style.visibility = 'hidden';
			document.getElementById("updateUserDtls:cityIdProfile").style.visibility = 'hidden';
			...
			document.getElementById("updateUserDtls:postofficeProfile").style.visibility = 'visible';
			document.getElementById("updateUserDtls:cityIdProfile").style.visibility = 'visible';
...
function alertForAccountLockEmailUpdate() {
	alert("Email ID updation will lead to deactivation of account till the new email id is verified.");
}
function alertForAccountLockMobileUpdate() {
	alert("Mobile Number updation will lead to deactivation of account till the new mobile number is verified.");
}
...
function fetchPostOfficeDistrictProfile() {
	var pincode = document.getElementById("updateUserDtls:postofficeProfile").value;
		if(pincode == "other-other"){
		} else{
			otherPin();
		}
...
function fetchPostOfficeListProfile() {
	var cityName = document.getElementById("updateUserDtls:cityIdProfile").value;
	getPostOfficeFromCity(cityName);
}

function validateProfile() {
	//alert("hello without js");
    var illegalChars = /\W/; 
    var illegalCharsPas = /[\W_]/;
   ...
   // alert(pin.val());
    ...
    var pancardMandateFlg = $("#updateUserDtls\\:pancardMandateFlgProfile"); 
       if(secAnswer.val() == '' )  {
        ...
        alert("Only letters and numbers are allowed in pancard number");
        pancardNumber.focus();
           return false;
        }
   	 }	     
   ...
   if(country.val() != 94 )  {
       alert("Please select India as country");
       country.focus();
       return false;
  }
   if(pin.val() == '' )  {
